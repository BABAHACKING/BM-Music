console.log('Baba Music – Initialized (Reverted Subfolder V2)');
...
async function getSongs() {
    ...
    const CACHE_KEY = 'babaSongsCache_v3_revert'; // cache-buster
    ...
    try {
        if (window.supabaseClient) {
            console.log('Fetching songs from cloud (Fresh)...');
            ...
            if (error) {
                ...
                window.babaError?.("Database Error: " + error.message);
                throw error;
            }
            if (data) {
                console.log(`Fetched ${data.length} songs from cloud.`);
                sessionStorage.setItem(CACHE_KEY, JSON.stringify(data));
                return data;
            }
        } else {
            console.error("Supabase Client missing!");
            window.babaError?.("Supabase Disconnected! Check internet.");
        }
        return [];
    } catch (error) {
        console.error("Error fetching songs:", error);
        window.babaError?.("Fetch Failed: " + error.message);
        return [];
    }
}
...
async function waitForSupabase(retries = 10, delay = 500) {
    for (let i = 0; i < retries; i++) {
        if (window.supabaseClient) return true;
        console.log(`Waiting for Supabase client... (${i + 1}/${retries})`);
...
async function main() {
    try {
        ...
        if (!supabaseReady) {
            console.error('CRITICAL: Supabase init timeout.');
            window.babaError?.('Connection Failed: Cloud config missing.');
        }
        ...
        if (songs.length > 0) {
            ...
            console.warn('⚠️ No songs returned from getSongs()');
            window.babaError?.('No songs found in cloud library.');
        }

        // Populate sidebar song list with heart buttons
        const songListContainer = document.querySelector(".songlist");
        if (!songListContainer) {
            console.error('❌ CRITICAL: .songlist element not found in DOM');
            return;
        }
        let songUL = songListContainer.getElementsByTagName("ul")[0];
        if (!songUL) {
            console.error('❌ CRITICAL: ul element not found in .songlist');
            return;
        }

        songUL.innerHTML = ""; // Clear existing
        console.log(`Populating library with ${songs.length} items...`);
